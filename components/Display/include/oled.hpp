...
class TalkDisplay {
   public:
    ...
    static void talk_task(void *pvParameters) {
        ...
        // 実行フラグをfalseへ変更
        // Ensure I2S is released even if we delete the task (avoid missing C++ destructors)
        // so that re-entering Game can re-initialize audio properly.
        {
            Max98357A cleanup; // dummy handle to satisfy scope; does not allocate
        }
...
class Composer {
   public:
    ...
    static void composer_task(void* pv) {
        ...
        for (int i = 0; i < STEPS; ++i) { p1[i] = -1; p2[i] = -1; nz[i] = -1; }
        // Small default groove
        p1[0] = 60; p1[4] = 64; p1[8] = 67; p1[12] = 72;
        nz[4] = 3; nz[12] = 3;
        ...
        auto draw = [&]() {
            ...
            snprintf(hdr, sizeof(hdr), "BPM:%d %s", tempo, playing ? "PLAY" : "STOP");
            ...
            for (int g = 0; g <= STEPS; g += 4) {
                ...
                if (gx >= x0 && gx <= x0 + grid_w) {
                    sprite.drawFastVLine(gx, y0 - 1, row_h * 3 + 2, 0x7BEF /*gray*/);
                }
            }
            ...
            auto draw_row = [&](int row, const int* arr, bool noise=false) {
                for (int s = 0; s < STEPS; ++s) {
                    ...
                    int y = y0 + row * row_h;
                    bool on = arr[s] >= 0;
                    bool sel = (s == cur_step) && ((cur_chan == row) || (noise && cur_chan == 2));
                    ...
                    if (on) {
                        ...
                        if (sel) {
                            ...
                            sprite.fillRect(x+1, y+box_h-3, box_w-2, 2, 0x0000);
                        }
                    } else {
                        // outline cell
                        ...
                        if (sel) {
                            ...
                            sprite.fillRect(x+1, y+1, box_w-2, box_h-2, 0x7BEF);
                        }
                    }
                }
            };
        ...
        while (1) {
            ...
            if (tb.pushing && js.pushed_up_edge) {
                ...
                if (cur_chan == 0) {
                    ...
                    if (nz[cur_step] >= 0) nz[cur_step] = std::max(0, nz[cur_step] - 1);
                    else cur_noise = std::max(0, cur_noise - 1);
                }
                auto note_name = [](int midi)->std::string {
                    if (midi < 0) return std::string("--");
                    static const char* names[12] = {"C","C#","D","D#","E","F","F#","G","G#","A","A#","B"};
                    int n = midi % 12; if (n < 0) n += 12;
                    int oct = midi / 12 - 1; char buf[8]; snprintf(buf, sizeof(buf), "%s%d", names[n], oct);
                    return std::string(buf);
                };
                std::string label;
                if (cur_chan == 2) { const char* nm = (nz[cur_step]==0?"HH":(nz[cur_step]==1?"SN":"BD")); char b[8]; snprintf(b, sizeof(b), "%s", nm); label = b; s_popup_kind=1; s_popup_val=nz[cur_step]; }
            ...
            if (eb.pushed && eb.push_type == 'l') {
                ...
                while (1) {
                    ...
                    for (int i=1;i<=3;i++) {
                        ...
                        sprite.setTextColor(sel?0x000000u:0xFFFFFFu, sel?0xFFFFu:0x0000u);
            ...
            if (eb.pushed && eb.push_type == 's' && s_play_task == nullptr) {
                ...
                PlayArgs* args = (PlayArgs*)heap_caps_malloc(sizeof(PlayArgs), MALLOC_CAP_DEFAULT);
                new (args) PlayArgs();
                ...
                auto task = +[](void* pv) {
                    ...
                    // Streaming render to DMA buffers (no large PSRAM allocations)
                    struct Ctx {
                        chiptune::GBSynth* synth;
                        const chiptune::Pattern* pat;
                        int bpm;
                        float d1; float d2; bool ns;
                    ...
                    auto fill = +[](int16_t* dst, size_t max, void* u)->size_t {
                        ...
                        size_t w = c->synth->render_block(*c->pat, c->bpm, c->d1, c->d2, c->ns, c->st, dst, max, /*ch1_sine=*/true);
                        int step = c->st.step;
                    ...
                    const size_t total_samples = (size_t)step_samples * (size_t)a->pat.steps;
                    spk.play_pcm_mono16_stream(total_samples, 1.0f, a->abortp, fill, &ctx);
                    a->~PlayArgs();
                    heap_caps_free(a);
                    Composer::s_play_task = nullptr;
                    // Explicitly release I2S channel before task exit so others can use audio
                    spk.deinit();
                    vTaskDelete(NULL);
                };
                xTaskCreatePinnedToCore(task, "compose_play", 4096, args, 5, &s_play_task, 1);
                // Do not block UI; task will self-delete. We can't easily reset handle here,
                // so we just ignore until user presses again after a while.
                // A tiny debounce
        ...
        while (1) {
            ...
            for (int i = start; i <= end; i++) {
                ...
                if (settings[i].setting_name == "Develop") {
                    ...
                    if (bs.empty()) bs = "cute";
                    std::string shown = bs == std::string("majestic") ? "Majestic" : (bs == std::string("random") ? "Random" : "Cute");
                    std::string label = std::string("Boot Sound [") + shown + "]";
                    sprite.print(label.c_str());
            ...
            if (type_button_state.pushed &&
                ...
                // Simple boot sound selector: Type=cycle, Enter=preview, Back=save
                type_button.clear_button_state();
                enter_button.clear_button_state();
                back_button.clear_button_state();
                joystick.reset_timer();
                ...
                auto idx_of = [&](const std::string& s){ for (size_t i=0;i<opts.size();++i) if (opts[i]==s) return (int)i; return 0; };
                std::string cur = get_nvs((char*)"boot_sound"); if (cur.empty()) cur = "cute";
                int idx = idx_of(cur);
                while (1) {
                    sprite.fillRect(0, 0, 128, 64, 0);
                    sprite.setFont(&fonts::Font2);
                    ...
                    std::string name;
                    if (opts[idx]=="majestic") name = "Majestic";
                    else if (opts[idx]=="gb") name = "GB Synth";
                    else if (opts[idx]=="random") name = "Random";
                    else if (opts[idx]=="song1") name = "Song 1";
                    else if (opts[idx]=="song2") name = "Song 2";
                    else if (opts[idx]=="song3") name = "Song 3";
                    else name = "Cute";
                    ...
                    if (tbs.pushed) { idx = (idx+1) % (int)opts.size(); type_button.clear_button_state(); }
                    if (ebs.pushed) {
                        Max98357A sp;
                        if (opts[idx]=="majestic") boot_sounds::play_majestic(sp, 0.5f);
                        else if (opts[idx]=="gb") boot_sounds::play_gb(sp, 0.9f);
                        else if (opts[idx]=="song1") boot_sounds::play_song(sp, 1, 0.9f);
                        else if (opts[idx]=="song2") boot_sounds::play_song(sp, 2, 0.9f);
                        else if (opts[idx]=="song3") boot_sounds::play_song(sp, 3, 0.9f);
                        else if (opts[idx]=="random") {
                            uint32_t r = (uint32_t)(esp_timer_get_time() & 3);
                            if (r==0) boot_sounds::play_cute(sp, 0.5f);
                            else if (r==1) boot_sounds::play_majestic(sp, 0.5f);
                            else boot_sounds::play_gb(sp, 0.9f);
                        } else boot_sounds::play_cute(sp, 0.5f);
                        enter_button.clear_button_state();
                    }
                    vTaskDelay(10/portTICK_PERIOD_MS);
                }
                ...
                while (comp.running_flag) { vTaskDelay(100 / portTICK_PERIOD_MS); }
                sprite.setFont(&fonts::Font2);
                type_button.clear_button_state();
                type_button.reset_timer();
                joystick.reset_timer();
