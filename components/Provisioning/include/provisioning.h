...
#define QRCODE_BASE_URL "https://espressif.github.io/esp-jumpstart/qrcode.html"
    ...
    /* Handler for the optional provisioning endpoint registered by the
     * application. The data format can be chosen by applications. Here, we are
     * using plain ascii text. Applications can choose to use other formats like
     * protobuf, JSON, XML, etc.
     */
    ...
    void main() {
        ...
        if (!provisioned) {
            ...
            /* An optional endpoint that applications can create if they expect
             * to get some additional custom data during provisioning workflow.
             ...
             * This call must be made after starting the provisioning, and only
             * if the endpoint has already been created above.
             */
            wifi_prov_mgr_endpoint_register("custom-data",
                                            custom_prov_data_handler, NULL);

            /* Uncomment the following to wait for the provisioning to finish
             * and then release the resources of the manager. Since in this case
             * de-initialization is triggered by the default event loop handler,
             * we don't need to call the following */
            // wifi_prov_mgr_wait();
            // wifi_prov_mgr_deinit();
